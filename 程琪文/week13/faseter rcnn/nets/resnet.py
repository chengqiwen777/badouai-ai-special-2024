...
class BatchNormalization(Layer):
    def __init__(self, epsilon=1e-3, axis=-1, weights=None,
        ...
        self.beta_init = initializers.get(beta_initializer)
        self.gamma_init = initializers.get(gamma_initializer)
...
def identity_block_td(input_tensor, kernel_size, filters, stage, block, trainable=True):
    ...
    x = TimeDistributed(
        Conv2D(nb_filters2, (kernel_size, kernel_size), trainable=trainable, kernel_initializer='normal',
...
def conv_block_td(input_tensor, kernel_size, filters, stage, block, input_shape, strides=(2, 2), trainable=True):
    ...
    x = TimeDistributed(Conv2D(nb_filters2, (kernel_size, kernel_size), padding='same', trainable=trainable,
                               kernel_initializer='normal'), name=conv_name_base + '2b')(x)
    x = TimeDistributed(BatchNormalization(axis=bn_axis), name=bn_name_base + '2b')(x)
    x = Activation('relu')(x)

    x = TimeDistributed(Conv2D(nb_filters3, (1, 1), kernel_initializer='normal'), name=conv_name_base + '2c',
                        trainable=trainable)(x)
    x = TimeDistributed(BatchNormalization(axis=bn_axis), name=bn_name_base + '2c')(x)

    shortcut = TimeDistributed(
        Conv2D(nb_filters3, (1, 1), strides=strides, trainable=trainable, kernel_initializer='normal'),
        name=conv_name_base + '1')(input_tensor)
